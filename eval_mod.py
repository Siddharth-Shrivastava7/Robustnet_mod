...
def setup_loader():
    ...
    if args.dataset == 'cityscapes':
        ...
        test_set = args.dataset_cls.DarkZurich(args.split,
                                               transform=val_input_transform,
                                               target_transform=target_transform,
                                               eval_mode=eval_mode,
                                               eval_scales=eval_scales,
...
class RunEval():
    ...
    def inf(self, imgs, img_names, gt, inference, net, scales, pbar, base_img):
        ...
        if self.write_image:
            ...
            resized_prediction_pre_argmax = F.interpolate(torch.tensor(prediction_pre_argmax).unsqueeze(dim=0), size=(270,480), mode='bilinear', align_corners=False) ## (H/4 x W/4) size to be saved
